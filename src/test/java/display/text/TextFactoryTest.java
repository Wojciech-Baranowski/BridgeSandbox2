...
public class TextFactoryTest {
    ...
    public void make_text_test() {
        ...
        for(int i = 0; i < inputX.length; i++){
            inputTexts2[i] = new Text(inputTexts[i], inputX[i], inputY[i], inputZ[i], inputFont, inputColor);
        }
        TextFactory textFactory = new TextFactory();
        Text[] output = new Text[inputX.length];
        //when
        for(int i = 0; i < inputX.length; i++){
            output[i] = textFactory.makeText(inputTexts[i], inputX[i], inputY[i], inputZ[i], "HBE24", "red");
        }
        //then
        for(int i = 0; i < inputX.length; i++){
