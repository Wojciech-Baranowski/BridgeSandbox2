...
public class RectangleFactoryTest {
    ...
    public void make_rectangle_test() {
        ...
        for(int i = 0; i < inputX.length; i++){
            inputRectangles[i] = new Rectangle(inputX[i], inputY[i], inputZ[i], inputW[i], inputH[i], assets.getColor(inputColorNames[i]));
        }
        Rectangle[] output = new Rectangle[inputX.length];
        //when
        for(int i = 0; i < inputX.length; i++){
            output[i] = rectangleFactory.makeRectangle(inputX[i], inputY[i], inputZ[i], inputW[i], inputH[i], inputColorNames[i]);
        }
        //then
        for(int i = 0; i < inputX.length; i++) {
    ...
    public void make_framed_rectangle_test() {
        ...
        Rectangle[] inputRectangles = new Rectangle[inputX.length];
        for(int i = 0; i < inputX.length; i++){
            inputRectangles[i] = new FramedRectangle(inputX[i], inputY[i], inputZ[i], inputW[i], inputH[i], inputFrameThicknesses[i], inputColors[i], inputFrameColors[i]);
        }
        Assets assets = AssetsBean.getAssets();
        for(int i = 0; i < inputX.length; i++){
            assets.addColor(inputColorNames[i], inputColorValues[i]);
        ...
        for(int i = 0; i < output.length; i++){
            output[i] = rectangleFactory.makeFramedRectangle(inputX[i], inputY[i], inputZ[i], inputW[i], inputH[i], inputFrameThicknesses[i], inputColorNames[i], inputFrameColorNames[i]);
        }
        //then
        for(int i = 0; i < output.length; i++){
            assertArrayEquals(inputRectangles[i].getP(), output[i].getP());
