...
public class ArrayBasedGameState implements GameState {
    ...
    public void doMove(int cardToPlayIndex) {
        ...
        if (this.initialNumberOfCards == this.numberOfPlayedCards) {
            summarizeGame();
        }
    }
    ...
    private void summarizeGame() {
        ...
        if (this.results[startingPairPoints][0] == -1) {
            for (int i = 0; i < this.numberOfPlayedCards; i++) {
    ...
    public int[] getIndicesOfPlayableCardsOnCurrentPlayerHand() {
        ...
        for (int i = 0; i < this.numberOfHandsCards[this.currentPlayer]; i++) {
            ...
            boolean playerHasVoidInStartingPlayerTableCardColor = this.numberOfHandsCardsPerColor[this.currentPlayer][this.tableCards[this.startingPlayer] / FIGURE_NUMBER] == 0;
    ...
    public boolean isNSOnMove() {
        return this.currentPlayer % 2 == 0;
    }

    public boolean isPairOnMoveTwiceInARow() {
        return this.previousStartingPlayers[this.numberOfPreviousStartingPlayers - 1] % 2 != this.startingPlayer % 2;
    }
}
